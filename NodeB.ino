/*
 ================================================================
  MESHCHAT NODE B — COMPLETE SYSTEM
  Same features as Node A. DHT11 also on Pin 4.
 ================================================================
*/
...
#define MY_NODE_NAME "B"
#define MY_NODE_ID    1
#define AP_SSID      "MeshChat-B"
...
bool      nodeOnline[3]={false,true,false};
...
esp_now_peer_info_t peerA,peerC;
...
void sendAll(MeshPacket& p){ espSend(MAC_A,p); espSend(MAC_C,p); }
...
void pushFirebase(){
  if(!wifiOK) return;
  HTTPClient http;
  String url=String(FIREBASE_URL)+"/nodes/B.json";
...
void hStatus(){
  ...
  for(int i=0;i<3;i++) if(i!=1&&nodeOnline[i]&&(now-lastSeen[i])>NODE_TIMEOUT) nodeOnline[i]=false;
...
void hSend(){
  ...
  if(gemQ){
    ...
    else if(strcmp(to,"A")==0){
      if(esp_now_send(MAC_A,(uint8_t*)&p,sizeof(p))!=ESP_OK){
...
void setup(){
  ...
  addP(peerA,MAC_A); addP(peerC,MAC_C);
...
// ── HTML — identical UI, same buildHTML() function ───────────
String buildHTML(){
